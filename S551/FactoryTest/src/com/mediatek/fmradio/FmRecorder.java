...
public class FmRecorder implements MediaPlayer.OnCompletionListener,
    ...
    public void startRecording(Context context) {
        ...
        stringBuilder.append(RECORDING_FILE_PREFIX).append("_").append(time)
    ...
    public void startPlayback() {
        ...
        try {
            ...
            Log.e(TAG, "Exception while trying to playback recording file: "
                    + e);
     ...
     *            The context
     * @param newName
     *            The name to override default recording name
     */
    public void saveRecording(Context context, String newName) {
        ...
        if (null != newName && !newName.equals(getRecordingName())) {
            File sdFile = new File(parentFile.getPath(), newName
            ...
            if (null != parentFile) {
                ...
                mRecordFile = new File(parentFile.getPath(), newName
                        + RECORDING_FILE_EXTENSION);
            }
        }
     ...
     *            the callback
     */
    public void registerRecorderStateListener(
            OnRecorderStateChangedListener listener) {
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        ...
        int error = (MediaPlayer.MEDIA_ERROR_SERVER_DIED == what) ? what
                : ERROR_PLAYER_INTERNAL;
        setError(error);
        synchronized (this) {
            if (null != mPlayer) {
                mPlayer.release();
    ...
    public void addCurrentRecordingToDb(final Context context) {
        ...
        MediaScannerConnection.scanFile(context,
                new String[] { mRecordFile.getAbsolutePath() }, null, null);
    }
    ...
    private int getPlaylistId(final Context context) {
        Cursor playlistCursor = context.getContentResolver().query(
                ...
                new String[] { FmRadioUtils.getPlaylistPath(context)
                        + RECORDING_FILE_SOURCE }, null);
        int playlistId = -1;
    ...
    private int addToMediaDB(final Context context, final ContentValues cv) {
        ...
        Cursor existItems = context.getContentResolver().query(
                ...
                new String[] { "_id" }, MediaStore.Audio.Media.DATA + "=?",
                new String[] { mRecordFile.getAbsolutePath() }, null);
        ...
        if (null != existItems) {
            try {
                ...
                if (existItems.moveToFirst()) {
                    ...
                    Log.d(TAG, "existing items update recording id"
                            + recordingId);
                    ...
                    if (null != insertResult) {
                        recordingId = Integer.valueOf(insertResult
                                .getLastPathSegment());
                    }
                }
            } finally {
    ...
    private void addToPlaylist(final Context context, final int playlistId,
        ...
        if (null != itemInMembers) {
            try {
                if (itemInMembers.moveToFirst()) {
                    ...
                    Log.d(TAG,
                            "addToPlaylist new item already in playlists.members table");
                    return;
                }
            } finally {
                itemInMembers.close();
        ...
        Cursor membersCursor = context.getContentResolver().query(membersUri,
