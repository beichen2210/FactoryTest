...
import com.mlt.factorytest.item.nfc.NfcEmReqRsp.NfcEmReq;
   ...
   // protected Socket mSocket; 
    ...
    public synchronized int sendCommand(int msgType, NfcEmReq cmdReq) {
        if (mSocket == null || mOutputStream == null || !isConnected()) {
            Log.i(NfcMainPage.TAG, "[NfcClient]send command fail");
            return -1;
        }
        Log.i(NfcMainPage.TAG, "[NfcClient]Send command type: " + msgType);
        try {
            ...
            if (cmdReq == null) {
                ...
                Log.i(NfcMainPage.TAG, "[NfcClient]Send command lenght: " + msglen);
            ...
            Log.i(NfcMainPage.TAG, "[NfcClient]sendCommand IOException: " + e.getMessage());
            e.printStackTrace();
            return -1;
        }
        return msgType;
    ...
    public boolean closeConnection() {
        Log.i(NfcMainPage.TAG, "[NfcClient]closeConnection().");
        if (mSocket == null || !isConnected()) {
            Log.i(NfcMainPage.TAG, "[NfcClient]close connection fail");
            return false;
        }
        ...
        try {
            ...
            Log.i(NfcMainPage.TAG, "[NfcClient]closeConnection IOException: " + e.getMessage());
        }
        if (null != mSocket) {
            try {
                mSocket.close();
            } catch (IOException e) {
                Log.i(NfcMainPage.TAG, "[NfcClient]closeConnection finally IOException: "
                        + e.getMessage());
            }
            mSocket = null;
        }
        sInstance = null;
        setConnected(false);
        Log.i(NfcMainPage.TAG, "[NfcClient]close connection success");
        return true;
    }

    // @Override
    public boolean createConnection(Context context) {
        Log.i(NfcMainPage.TAG, "[NfcClient]createConnection().");
        int timeout = 30000;
        if (mSocket != null && isConnected()) {
            Log.i(NfcMainPage.TAG, "[NfcClient]createConnection: has connected");
            return true;
        }
        try {
          ...
          /*  Log.i(NfcMainPage.TAG, "[NfcClient]Is createConnection IOException: " + e.getMessage());*/
            mSocket = null;
        }
        if (mSocket == null) {
            return false;
        ...
        try {
            ...
            Log.i(NfcMainPage.TAG, "[NfcClient]getStream IOException: " + e.getMessage());
            try {
                mSocket.close();
                mSocket = null;
