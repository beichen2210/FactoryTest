...
public class ShellExe {
    ...
    public static int execCommand(String command) throws IOException {
    	Log.i(NfcMainPage.TAG,"[ShellExe]"+ "execCommand");
    ...
    public static int execCommand(String[] command) throws IOException {
    	Log.i(NfcMainPage.TAG,"[ShellExe]"+ "execCommand");
        int result = RESULT_FAIL;//RESULT_FAIL=-1
        ...
        try {
            ...
            if (proc.waitFor() == 0) {
            	Log.i(NfcMainPage.TAG,"[ShellExe]"+ "waitFor");
                String line = bufferedReader.readLine();
                Log.i(NfcMainPage.TAG,"[ShellExe]-line:"+ line);
                if (line != null) {
                ...
                Log.i(NfcMainPage.TAG, "exit value = " + proc.exitValue());
                sResultBuilder.append(ERROR);
                result = RESULT_FAIL;
            }
        } catch (InterruptedException e) {
            Log.i(NfcMainPage.TAG, "exe shell command InterruptedException: "
                    + e.getMessage());
            sResultBuilder.append(ERROR);
            ...
            if (null != bufferedReader) {
                try {
                    ...
                    Log.i(NfcMainPage.TAG, "close reader in finally block exception: " + e.getMessage());
                }
            }
        }
        return result;
