...
import android.os.IBreathLedsService;  //cqf add MOPLUES-4 20151112
/**
...
public final class BatteryService extends SystemService {
    ...
    public void onStart() {
        ...
        //chb add for MOPLUES-34 2015-11-23 17:46:49
        IntentFilter factoryTestFilter = new IntentFilter();
        factoryTestFilter.addAction(FACTORYTEST_CLOSE_BREATHLIGHT);
        factoryTestFilter.addAction(FACTORYTEST_OPEN_BREATHLIGHT);
        factoryTestFilter.addAction(FACTORYTEST_STOP_BREATHLIGHT);
        mContext.registerReceiver(mFactoryTestReceiver, factoryTestFilter);
        //chb add for MOPLUES-34 2015-11-23 17:46:59
    }
    
    //chb add for MOPLUES-34 begin 2015-11-25 13:58:03
    BroadcastReceiver mFactoryTestReceiver  = new BroadcastReceiver() {
       @Override
       public void onReceive(Context mContext, Intent mIntent) {
           String action = mIntent.getAction();
           if (action.equals(FACTORYTEST_OPEN_BREATHLIGHT)) {
               mIsOpenBreathLight = true;
               mIsCloseBreathLight = false;
               mLed.updateLightsLocked();
           } else if (action.equals(FACTORYTEST_CLOSE_BREATHLIGHT)) {
               mIsOpenBreathLight = false;
               mIsCloseBreathLight = true;
               mLed.updateLightsLocked();
           } else if (action.equals(FACTORYTEST_STOP_BREATHLIGHT)) {
               mIsOpenBreathLight = false;
               mIsCloseBreathLight = false;
           }
       }
    };
    //chb add for MOPLUES-34 end  2015-11-25 13:58:07
    ...
    private final class Led {
        ...
        private IBreathLedsService mBreathLedSvc = null; //cqf add MOPLUES-4 20151112
        ...
        //cqf add MOPLUES-4 20151112 start
        public void breathLedAction(int index) {
            if (mBreathLedSvc == null) {
                mBreathLedSvc = IBreathLedsService.Stub.asInterface(ServiceManager.getService("breath_leds"));
            }
            try {
                if (mBreathLedSvc != null) {
                    mBreathLedSvc.ledBreath(index);
                }
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        ...
        public void twinkleLedAction(int index) {
            if (mBreathLedSvc == null) {
                mBreathLedSvc = IBreathLedsService.Stub.asInterface(ServiceManager.getService("breath_leds"));
            }
            ...
            try {
                if (mBreathLedSvc != null) {
                    mBreathLedSvc.ledTwinkle(index);
                }
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        ...
        public void closeBreathLed(int index) {
            if (mBreathLedSvc == null) {
                mBreathLedSvc = IBreathLedsService.Stub.asInterface(ServiceManager.getService("breath_leds"));
            }
            ...
            try {
                if (mBreathLedSvc != null) {
                    mBreathLedSvc.turnOffLeds(index);
                }
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
        //cqf add MOPLUES-4 20151112 end
        ...
        public void updateLightsLocked() {
            ...
            if (level < mLowBatteryWarningLevel) {
                if (status == BatteryManager.BATTERY_STATUS_CHARGING) {
                    ...
                    breathLedAction(1);  //cqf add MOPLUES-4 20151112
                ...
                if (status == BatteryManager.BATTERY_STATUS_FULL || level >= /*90*/100) {
                    updateLedStatus();
                    // Solid green when full or charging and nearly full
                    mBatteryLight.setColor(mBatteryFullARGB);

                    closeBreathLed(1); //cqf add MOPLUES-4 20151112
            ...
            //chb add for MOPLUES-34 2015-11-25 13:58:53            
            if (mIsOpenBreathLight && (!mIsCloseBreathLight)){
                mBatteryLight.setColor(mBatteryMediumARGB);
                breathLedAction(1);  
            } else if ((!mIsOpenBreathLight) && mIsCloseBreathLight){ 
                //updateLedStatus();
                mBatteryLight.setColor(mBatteryFullARGB);
                closeBreathLed(1);
            }
            //chb add for MOPLUES-34 2015-11-25 13:58:58           
        }
    ...
    //chb add for MOPLUES-34 begin 2015-11-23 17:40:56
    ...
    private boolean mIsOpenBreathLight = false;
    private boolean mIsCloseBreathLight = false;
    //chb add for MOPLUES-34 end 2015-11-23 17:56:50
