...
//tiandajiao add for MTSFEFL-17 start 20150313
...
public class SpecialCharSequenceMgr {
    ...
    /// @}
    //chb add for VFOZ-1:add factorytest Start command 2015-7-27 begin
    private static final String FACTORYTEST_MODE_MALATA_ENTRY = "*953*2#";
    //chb add for VFOZ-1:add factorytest Start command 2015-7-27 end
    //chb add for Run In:VFOZ-58 2015-8-17 begin
    private static final String FACTORYAUTOTEST_MODE_MALATA_ENTRY = "*#*#309#*#*";
    //chb add for Run In:VFOZ-58 2015-8-17 end 

	private static final String CSD_EM_MODE_MALTA_ENTRY = "*#*#310#*#*";//pss add for VFOZBENQ-15 20150821
    ...
    public static boolean handleChars(Context context, String input, EditText textField) {
        ...
        if (handleDeviceIdDisplay(context, dialString)
                ...
                || handleFactoryResetSettings(context,dialString)//tiandajiao add MTSFEFL-17 for reset factory settings version 20150313
                || handleQueryInternalVersionEntry(context,dialString) //tiandajiao add MTSFEFL-17 for query internal version 20150313
                || handleEngineerModeEntry(context,dialString) //tiandajiao add MTSFEFL-17 for ENGINEER MODE 20150313
                || handleSecretCode(context, dialString)
         	 || handleFactoryTestModeEntry(context,dialString) //chb add for VFOZ-1:add factorytest Start command 2015-7-27 
                /// M: for OP09 6M project @{
	...
	//chb add for Run In:VFOZ-58 2015-8-17 begin
	if (len > 8 && input.equals(FACTORYAUTOTEST_MODE_MALATA_ENTRY)){
		Intent intentFa = new Intent("com.mlt.factoryautotest.action.TEST_LIST");
		intentFa.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		context.startActivity(intentFa);		
		return true;
	}
	//chb add for Run In:VFOZ-58 2015-8-17 end
        if (len > 8 && input.startsWith("*#*#") && input.endsWith("#*#*")) {
			...
			if(input.equals(CSD_EM_MODE_MALTA_ENTRY)){
				Intent intentCsd = new Intent(
                        "com.mlt.csdem.action.TEST_CSD");
                intentCsd.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                context.startActivity(intentCsd);
    ...
    static boolean handleDeviceIdDisplay(Context context, String input) {
        ...
        if (telephonyManager != null && input.equals(MMI_IMEI_DISPLAY)) {
			//yutianliang delete for VFOZBENQ-81 begin
			/*
            int labelResId = (telephonyManager.getPhoneType() == TelephonyManager.PHONE_TYPE_GSM) ?
                    R.string.imei : R.string.meid;
			*/
			//yutianliang delete for VFOZBENQ-81 end
			int labelResId = R.string.imei;//yutianliang add for VFOZBENQ-81
