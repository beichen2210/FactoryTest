...
//    private int mRecordState = FmRecorder.STATE_INVALID;
    ...
    private String CLOSEFM="com.close.fm";
    ...
    private class FmServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.d("hgx", "onReceive, action = " + action + " / command = " + command);
            ...
            if ((SOUND_POWER_DOWN_MSG.equals(action) && CMDPAUSE.equals(command))) {
                ...
                Log.d(TAG, "onReceive, mValueHeadSetPlug ="+mValueHeadSetPlug+" mIsServiceInited="+mIsServiceInited);
                switchAntennaAsync(mValueHeadSetPlug);
                Log.d(TAG, "onReceive, mIsServiceInited ="+mIsServiceInited);
...
//    }
    ...
    private void registerAudioPortUpdateListener() {
        ...
        if (mAudioPortUpdateListener == null) {
    		Log.d(TAG, "registerAudioPortUpdateListener==null register seccess !");	
    ...
    private synchronized void createAudioPatch() {
    	Log.d(TAG, "createAudioPatch "+" isAntennaAvailable="+isAntennaAvailable()+" getForceUse() ="+getForceUse());
        if (mIsPlaying192KHz) {
    		Log.d(TAG, "mIsPlaying192KHz");
            releaseAudioPatch(); 
            if (!mIsRender) {
                startRender();
            }
        ...
        if (mAudioPatch == null) {
    		...
    		Log.d(TAG, "go to createAudioPatchByEarphone()");
                stopAudioTrack();
                stopRender();
                createAudioPatchByEarphone();
            } else if (isPatchMixerToSpeaker(patches)) {
    		Log.d(TAG, "go to createAudioPatchBySpeaker()");
                stopAudioTrack();
                stopRender();
                createAudioPatchBySpeaker();
            } else {
    		Log.d(TAG, "go to startRender()");
                startRender();
            }
        }
    }
